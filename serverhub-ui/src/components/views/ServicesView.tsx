...
import { Play, Square, RotateCcw, Search, Loader2 } from 'lucide-react';
import { apiRequest } from '../../api/client';

async function serviceAction(name: string, action: 'start' | 'stop' | 'restart') {
  await apiRequest(`/api/services/${encodeURIComponent(name)}/action`, {
    method: 'POST',
...
export default function ServicesView() {
  ...
  const doAction = async (name: string, action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      ...
      setError(`${action} ${name}: ${(e as Error).message}`);
    } finally {
      setPending((p) => {
        const next = { ...p };
        delete next[name];
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)] bg-[var(--bg-secondary)] flex-shrink-0">
        ...
        {error && (
          <span className="text-xs text-[var(--accent-red)] ml-auto truncate max-w-xs" title={error}>{error}</span>
        )}
      </div>
            ...
            <div key={svc.name} className="grid items-center px-4 py-3 border-b border-[var(--border-subtle)] hover:bg-[var(--bg-hover)] text-sm"
              ...
              <div className="flex items-center gap-1 justify-end">
                {busy ? (
                  ...
                  <>
                    <button
                      className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--bg-card)] hover:text-[var(--accent-green)] transition-colors disabled:opacity-40"
                      title="Start"
                    ...
                    <button
                      className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--bg-card)] hover:text-[var(--accent-red)] transition-colors disabled:opacity-40"
                      title="Stop"
                    ...
                    <button
                      className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--bg-card)] hover:text-[var(--accent-yellow)] transition-colors"
                      title="Restart"
