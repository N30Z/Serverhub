...
import { Plus, Play, Pause, Trash2, Edit2, Clock, CheckCircle, XCircle, Loader2, ChevronDown } from 'lucide-react';
...
function CronJobRow({ job, onEdit }: { job: CronJob; onEdit: (job: CronJob) => void }) {
  ...
  return (
    <div className="grid items-center px-4 py-3 border-b border-[var(--border-subtle)] hover:bg-[var(--bg-hover)] transition-all text-sm"
      style={{ gridTemplateColumns: '1.5rem 1fr 12rem 10rem 8rem 7rem 7rem 6rem' }}>
      ...
      <div className="flex items-center gap-1 justify-end">
        <button className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--bg-card)] hover:text-[var(--accent-green)] transition-colors" title="Run now">
          <Play size={12} />
        </button>
        ...
        <button className="w-7 h-7 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--bg-card)] hover:text-[var(--accent-red)] transition-colors" title="Delete">
          <Trash2 size={12} />
        </button>
      </div>
    </div>
...
function CronEditorModal({ job, onClose }: { job: CronJob | null; onClose: () => void }) {
    ...
    <div className="modal-overlay animate-fade-in" onClick={onClose}>
      <div className="modal w-[560px] max-w-[95vw] animate-slide-in-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-[var(--border)]">
          ...
          <button className="btn-primary text-xs">{job ? 'Save Changes' : 'Create Job'}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {cronJobs.map((job) => (
          <CronJobRow key={job.id} job={job} onEdit={setEditingJob} />
        ))}
      </div>

      {editingJob !== undefined && (
        <CronEditorModal job={editingJob} onClose={() => setEditingJob(undefined)} />
      )}
    </div>
  );
}
