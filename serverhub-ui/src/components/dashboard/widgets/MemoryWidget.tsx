...
export function MemoryWidget({ onOpenDetail, isEditing }: Props) {
    ...
    <div className="group h-full" onClick={!isEditing ? onOpenDetail : undefined}>
      <WidgetBase title="Memory">
        <div className="flex items-end justify-between mb-2">
          <div>
            <div className="metric-value" style={{ color }}>{memory.usage.toFixed(1)}<span className="text-base font-normal text-[var(--text-secondary)]">%</span></div>
            <div className="text-[11px] text-[var(--text-muted)] mt-0.5">{formatBytes(memory.used * 1024 * 1024)} / {formatBytes(memory.total * 1024 * 1024)}</div>
          </div>
          <div className="text-right text-[11px]">
            <div className="text-[var(--text-muted)]">Swap</div>
            <div className="text-[var(--text-secondary)]">{formatBytes(swap.used * 1024 * 1024)}</div>
          </div>
        </div>
        <GaugeBar value={memory.usage} color={color} />
        <div className="mt-1.5 flex gap-3 text-[10px] text-[var(--text-muted)]">
          <span>Cache <span className="text-[var(--text-secondary)]">{formatBytes(memory.cached * 1024 * 1024)}</span></span>
          <span>Buf <span className="text-[var(--text-secondary)]">{formatBytes(memory.buffers * 1024 * 1024)}</span></span>
        </div>
